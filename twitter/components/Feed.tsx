  ...
  const fetchTweets = async () => {
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        const mappedTweets = res.data.map((t: any) => ({
          ...t,
          id: t._id || t.id
        }));
      ...
      console.error("Failed to fetch tweets, using mock data", error);
      setTweetsData(tweets);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen">
      ...
      <div className="divide-y divide-gray-800">
        {loading ? (
          ...
          tweetsData.map((tweet: any) => <TweetCard key={tweet._id || tweet.id} tweet={tweet} />)
        )}
      </div>
    </div>
  );
