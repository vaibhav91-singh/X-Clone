...
export default function AuthModal({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) {
  ...
  const [deviceInfo, setDeviceInfo] = useState<{ browser: string; os: string; device: string } | null>(null);
    ...
    try {
      if (mode === 'login') {
        ...
        if (response.requiresOTP) {
          // Chrome browser - show OTP verification screen
          setMode('otp-verify');
          setFormData(prev => ({ 
            ...prev, 
            loginRecordId: response.loginRecordId || '' 
          }));
          setDeviceInfo(response.deviceInfo || null);
        } else {
          // Microsoft browser or non-Chrome - direct login
          onClose();
          setFormData({ email: '', password: '', username: '', displayName: '', otp: '', loginRecordId: '' });
          setErrors({});
        }
      } else if (mode === 'otp-verify') {
        // Verify OTP for Chrome login
        await verifyLoginOTP(formData.email, formData.otp, formData.loginRecordId);
        onClose();
        setFormData({ email: '', password: '', username: '', displayName: '', otp: '', loginRecordId: '' });
        setErrors({});
        setMode('login');
  ...
  if (mode === 'otp-verify') {
    return (
      <div className="fixed inset-0 bg-background/60 flex items-center justify-center z-50 p-4">
        <Card className="w-full max-w-md bg-background border-border text-foreground">
          <CardHeader className="relative pb-6">
            <Button
              variant="ghost"
              size="icon"
              className="absolute right-4 top-4 text-foreground hover:bg-accent"
              onClick={() => {
                setMode('login');
                setErrors({});
                setFormData({ email: '', password: '', username: '', displayName: '', otp: '', loginRecordId: '' });
            ...
            <div className="text-center">
              <div className="mb-6 flex justify-center">
                <TwitterLogo size="xl" className="text-foreground" />
              </div>
            ...
            {deviceInfo && (
              <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-4">
                <p className="text-sm text-blue-400 mb-2">
                  <strong>Chrome Login Detected</strong>
                </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="text-center text-2xl tracking-widest font-mono bg-transparent border-border text-foreground"
                  disabled={isLoading}
                  autoFocus
                />
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-10 bg-transparent border-border text-foreground placeholder-muted-foreground focus:border-blue-500"
                      disabled={isLoading}
                    />
                  </div>
                  {errors.displayName && (
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-8 bg-transparent border-border text-foreground placeholder-muted-foreground focus:border-blue-500"
                      disabled={isLoading}
                    />
                  </div>
                  {errors.username && (
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 pr-10 bg-transparent border-border text-foreground placeholder-muted-foreground focus:border-blue-500"
                  disabled={isLoading}
                />
                <Button
                  type="button"
